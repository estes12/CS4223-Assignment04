...
{
    public InputField NewScores;
    public InputField PlayerNames;
    public int num_scores = 5;

    public void AddNewScore()
    {
        string path = "Assets/Resources/NewScores.txt";
        string line;
        string[] fields;
        int scores_written = 0;
        string playerNames = "don't forget to input";
        string newScores = "999";
        bool newScoreWritten = false;
        string[] writeNames = new string[5];
        string[] writeScores = new string[5];

        playerNames = PlayerNames.text;
        newScores = NewScores.text;
        ...
        while (!reader.EndOfStream )
        {
            ...
            if (!newScoreWritten && scores_written < num_scores) // if new score has not been written yet
            {
                ...
                if(Convert.ToInt32(NewScores) > Convert.ToInt32( fields[1]))
                {
                    writeNames[scores_written] = playerNames;
                    writeScores[scores_written] = newScores;
        ...
        TextAsset asset = (TextAsset)Resources.Load("NewScores");
